...
class StoriSerie {
    ...
    public function insert($fields = array()) {
        ...
        if (!$this->_db->insert('stori_serie', $fields)) {
...
// USER DATA UPDATE
    public function update($fields = array(), $id = null) {
        if (!$this->_db->update('stori_serie', $id, $fields)) {
...
// FIND USER
    public function find($user = null) {
        if ($user) {
            ...
            if (is_numeric($user)) {
                $field = 'ID';
                $data = $this->_db->get(array(), 'stori_serie', array($field, '=', $user));
                if ($data->count()) {
...
// FIND USER
    public function find_user($user = null, $fields = array()) {
        ...
        if ($user) {
            ...
            if (is_numeric($user)) {
                ...
                $subscriberClass = new StoriSerie();
    ...
    public function select($sql = null) {
        $data = $this->_db->query("SELECT* FROM `stori_serie` {$sql}");
        if ($data->count()) {
