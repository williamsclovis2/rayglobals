...
class UploadHandler {
    ...
    protected function get_file_objects($iteration_method = 'get_file_object') {
        ...
        $lists_files = array_values(array_filter(array_map(
                                array($this, $iteration_method),
                                scandir($upload_dir)
        )));
        $lists_files = array_reverse($lists_files);
        return $lists_files;
    }
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    $file_path = File::cleanVideoPath($file_path);
                    set_magic_temp_file($file_path); // magic
                    // $fileClass = new File();
                    // $fileClass->update(array(
                    // 'description'=>addslashes($file_path)
                    // ),6);

                    $upit = move_uploaded_file($uploaded_file, $file_path);
                }
            } else {
